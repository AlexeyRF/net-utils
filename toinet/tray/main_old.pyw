...
def toggle_proxy(icon, item):
    ...
    if not proxy_enabled:
        ...
        proxy_enabled = False
    icon.update_menu()
def manual_connect(icon, item): run_script("connector.pyw")
def manual_disconnect(icon, item): run_script("disconnector.pyw")
def toggle_auto_connection(icon, item):
    ...
    icon.update_menu()
def open_browser_settings(icon, item): open_inet_props()
def manual_settings(icon, item): run_script("maestro.pyw")
...
def build_menu():
    return Menu(
        MenuItem(lambda item: "Выключить прокси" if proxy_enabled else "Включить прокси", toggle_proxy),
        MenuItem("Подключиться вручную", manual_connect),
        MenuItem("Отключиться вручную", manual_disconnect),
        MenuItem(lambda item: "Отключить автоподключение" if auto_connection else "Включить автоподключение", toggle_auto_connection),
