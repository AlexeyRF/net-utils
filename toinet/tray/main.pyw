...
def read_auto_connection():
    if not os.path.exists(AUTO_CONNECTION_FILE):
        with open(AUTO_CONNECTION_FILE, "w") as f: f.write("true")
        return True
    with open(AUTO_CONNECTION_FILE, "r") as f: return f.read().strip().lower() == "true"
def write_auto_connection(value: bool):
    with open(AUTO_CONNECTION_FILE, "w") as f: f.write("true" if value else "false")
def run_script(script_name):
    if os.path.exists(script_name):
        subprocess.Popen([sys.executable, script_name], creationflags=subprocess.CREATE_NO_WINDOW)
        log(f"Started: {script_name}")
    else: log(f"Script not found: {script_name}")
...
def toggle_proxy():
    ...
    if not proxy_enabled:
        if not os.path.exists(TORRC_FILE): run_script_blocking("auto_maestro.pyw")
        run_script("launcher.pyw")
        if auto_connection: run_script("connector.pyw")
        proxy_enabled = True
    else:
        ...
        proxy_enabled = False
    update_menu()
def manual_connect(): run_script("connector.pyw")
def manual_disconnect(): run_script("disconnector.pyw")
def toggle_auto_connection():
    global auto_connection
    auto_connection = not auto_connection
    write_auto_connection(auto_connection)
    update_menu()
def open_browser_settings(): open_inet_props()
def manual_settings(): run_script("maestro.pyw")
def exit_app():
    run_script("disconnector.pyw")
    run_script("closer.pyw")
    log("Exiting...")
...
def update_menu():
    if toggle_proxy_action: toggle_proxy_action.setText("Выключить прокси" if proxy_enabled else "Включить прокси")
    if toggle_auto_action: toggle_auto_action.setText("Отключить автоподключение" if auto_connection else "Включить автоподключение")
def create_tray_menu():
    ...
    tray.show()
    update_menu()
    tray.showMessage("TOR TRAY", "Приложение запущено", QSystemTrayIcon.Information, 1000)
